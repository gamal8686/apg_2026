...
class _CreateLoginViewState extends State<CreateLoginView> {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      ...
      child: Builder(
        builder: (context) {
          final cubit = context.read<CreateLoginCubit>();
              ...
              child: Form(
                ...
                child: Column(
                  children: [
                    ...
                    BlocConsumer<CreateLoginCubit, StateCreateLogin>(
                      listener: (context, state) {
                        if (state is StateCreateLoginSuccess) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(content: Text('تم إنشاء الحساب بنجاح')),
                          );
                          goTo(
                            BlocProvider(
                              create: (_) => box<LoginCubit>(),
                              child: LoginView(),
                            ),
                          );
                        }

                        if (state is StateCreateLoginError) {
